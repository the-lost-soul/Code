...
void solve(int arr[] , int size)
{
     ...
     * only with  elements which have occurred odd number of times .
     * eg. -- [1 , 2 , 3 , 3 , 1 , 4 , 2 , 1 , 5 , 2 , 2 , 4 , 5 , 5]
     * When we will do XOR then XOR = 1^2^3^3^1^4^2^1^5^2^2^4^5^5 and we know
     * 0^1 = 1 and 0^0 = 0or 1^1 = 0 hence finally XOR = 1^5 .
     */
    int XORsum = 0 ;
    for(int i = 0 ; i < size ; i++)
        XORsum = XORsum ^ arr[i] ;
    ...
    if(set1 > set2)cout<<set1<<" "<<set2<<endl ;
    else cout<<set2<<" "<<set1<<endl ;
